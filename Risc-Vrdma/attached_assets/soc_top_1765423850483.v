...
module soc_top (
    input  wire         clk,
    input  wire         rst_n,
    // RDMA descriptor input (external producer)
    ...
    // AXI interconnect signals (connected between master and memory model)
    ...
    // Instantiate RDMA engine and connect its memory interface to AXI vector interface
    rdma_engine u_rdma (
        .clk(clk),
        .rst_n(rst_n),
        .desc_addr(desc_addr),
    ...
    // Instantiate AXI4 master to arbitrate scalar and vector
    axi4_master u_axi (
        .clk(clk),
        .rst_n(rst_n),
        // Scalar interface
        ...
        .scalar_rdata(scalar_rdata),
        .scalar_ready(scalar_ready),
        // Vector interface (connected to RDMA)
